...
// FIX 3: Isolated enrollment buffer to prevent memory race conditions during typing
...
void trigger_door_unlock() {
    ...
    broadcast_ws_msg(HTTPD_WS_TYPE_TEXT, (uint8_t*)unlock_msg, strlen(unlock_msg)); // Sync with dashboard
...
static void btn_event_cb(lv_event_t * e) {
    ...
    if(code == LV_EVENT_CLICKED) {
        ...
        if (strcmp(txt, "ADMIT") == 0 || strcmp(txt, "MANUAL UNLOCK") == 0) {
            ...
            broadcast_ws_msg(HTTPD_WS_TYPE_TEXT, (uint8_t*)deny_msg, strlen(deny_msg)); // FIX 2: Sync with dashboard
...
void build_ui() {
    ...
    lv_obj_set_size(ui_btn_unlock, 200, 50);
    lv_obj_align(ui_btn_unlock, LV_ALIGN_BOTTOM_MID, 0, -20);
...
void save_to_spiffs(const char* name, float* embedding) {
    ...
    // FIX 3: Safe, isolated saving of the enrollment image
    if (enroll_jpeg_buf) {
...
void ml_inference_task(void *pvParameters) {
    ...
    while (1) {
        if (xQueueReceive(ml_queue, &job, portMAX_DELAY)) {
            ...
            // Responsive Display Drawing
            if (xSemaphoreTakeRecursive(lvgl_mutex, portMAX_DELAY)) {
                img_sprite.drawJpg(job.jpeg_buf, job.jpeg_len, 0, 0);
                ...
                // Clear buttons if currently visible
                lv_obj_add_flag(ui_btn_admit, LV_OBJ_FLAG_HIDDEN); 
                lv_obj_add_flag(ui_btn_deny, LV_OBJ_FLAG_HIDDEN);
            ...
            if (detect_results.size() > 0) {
                ...
                if (current_mode == MODE_ENROLL) {
                    ...
                    if (highest_similarity > 0.80f) {
                        // FIX 1: Logging for Enrolled Users
                        ...
                        // FIX 3: Isolated buffer saves frame *only* when a new face is successfully detected
                        if (enroll_jpeg_buf) heap_caps_free(enroll_jpeg_buf);
                        enroll_jpeg_buf = (uint8_t*)heap_caps_malloc(job.jpeg_len, MALLOC_CAP_SPIRAM);
                        if (enroll_jpeg_buf) {
                            memcpy(enroll_jpeg_buf, job.jpeg_buf, job.jpeg_len);
                            enroll_jpeg_len = job.jpeg_len;
                        }
            ...
            // FIX 1: Seconds Precision Logging
    ...
    <script>
        ...
        function handleCommand(cmd) {
            ...
            if (cmd === "UI_MSG:NEW_FACE") {
                ...
                fetchUsers();
                resetControls();
            // FIX 2: Dashboard clears its own buttons when physical display triggers actions
        ...
        function denyAccess() {
            ...
            // FIX 2: Send DENY command to server to sync the physical display buttons
            ws.send("CMD:DENY"); 
            resetControls();
        }
        ...
        function renderUsers(userStr) {
            ...
            users.forEach(u => {
                if(u.trim() !== "") {
                    // Cache buster logic added to fetch fresh images
...
esp_err_t ws_handler(httpd_req_t *req) {
    ...
    if (ws_pkt.type == HTTPD_WS_TYPE_TEXT) {
        ...
        if (strncmp(text, "CMD:MODE_ENROLL", 15) == 0) {
            ...
            ESP_LOGI(TAG, "Mode set to ENROLLMENT"); // FIX 1
        } else if (strncmp(text, "CMD:MODE_SECURITY", 17) == 0) {
            current_mode = MODE_SECURITY;
            ESP_LOGI(TAG, "Mode set to SECURITY"); // FIX 1
            ...
            // FIX 2: Clear Display buttons when Dashboard triggers a Deny
            ...
            broadcast_ws_msg(HTTPD_WS_TYPE_TEXT, (uint8_t*)"CMD:DENY", 8); // Echo back to sync clients
        }
    ...
    if (ws_pkt.type == HTTPD_WS_TYPE_BINARY) {
        ESP_LOGI(TAG, "Received WS Binary Payload: %d bytes", ws_pkt.len);
        uint8_t* jpeg_buf = (uint8_t*)heap_caps_malloc(ws_pkt.len, MALLOC_CAP_SPIRAM);
        ws_pkt.payload = jpeg_buf;
        httpd_ws_recv_frame(req, &ws_pkt, ws_pkt.len);

        MLJob job; job.jpeg_buf = jpeg_buf; job.jpeg_len = ws_pkt.len;

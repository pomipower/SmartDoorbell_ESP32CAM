...
void init_gpio() {
    ...
    gpio_set_level(RELAY_PIN, 0); // Start OFF (0V)
...
void relay_task(void *pvParameters) {
    while(1) {
        ...
        gpio_set_level(RELAY_PIN, 1); // 3.3V out
...
static void wifi_event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) esp_wifi_connect();
        ...
        is_wifi_connected = false; esp_wifi_connect();
...
static void websocket_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    switch (event_id) {
        case WEBSOCKET_EVENT_CONNECTED:
            is_ws_connected = true; break;
        case WEBSOCKET_EVENT_DISCONNECTED:
            is_ws_connected = false; break;
        case WEBSOCKET_EVENT_DATA:
            if (data->op_code == 0x01) { 
                ...
                // If the S3 broadcasts the master UNLOCK string, trigger the relay!

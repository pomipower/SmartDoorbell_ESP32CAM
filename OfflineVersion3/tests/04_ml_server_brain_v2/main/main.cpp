...
float pending_feature[128]; // Holds a new face temporarily until the UI provides a name
...
<body>
    ...
    <div id="admin" class="content">
        ...
        <button class="btn-danger" onclick="wipeDB()">⚠️ DELETE ALL ENROLLMENTS</button>
    </div>
    ...
    <script>
        ...
        function initWS() {
            ...
            ws.onopen = () => document.getElementById('ws-status').innerHTML = '<span style="color:#4CAF50;">Live</span>';
            ...
            ws.onmessage = function(e) {
                if (typeof e.data === 'string') {
                    ...
                    document.getElementById('camImage').src = URL.createObjectURL(e.data);
                }
            };
        }
        ...
        function handleCommand(cmd) {
            ...
            if (cmd === "UI_MSG:NEW_FACE") {
                ...
                status.innerHTML = `[${dateStr}] ✅ Welcome, <b>${cmd.split(":")[2]}</b>! Door Unlocked.`;
                resetControls();
            } else if (cmd === "UI_MSG:UNKNOWN") {
                status.innerHTML = `[${dateStr}] ❌ UNKNOWN PERSON DETECTED!`;
                controls.innerHTML = `<button onclick="manualUnlock()">Admit</button> <button class="btn-danger" onclick="resetControls()">Deny</button>`;
                controls.style.display = "block";
        ...
        function acceptFace() {
            ...
            if(name && name.trim() !== "") {
                ws.send("CMD:SAVE_NAME:" + name.trim());
            }
        }
        ...
        function wipeDB() {
            if(confirm("Are you sure? This will delete all faces permanently!")) {
                ws.send("CMD:WIPE_DB");
            }
        }
...
// Broadcasts a message to all connected Web UI clients (and the CAM)
...
void ml_inference_task(void *pvParameters) {
    ...
    while (1) {
        if (xQueueReceive(ml_queue, &job, portMAX_DELAY)) {
            // INSTANTLY broadcast the raw JPEG to the Web UI for visual feedback
            ...
            if (detect_results.size() > 0) {
                ...
                if (current_mode == MODE_ENROLL) {
                    float highest_similarity = 0.0f; char best_match[32] = "";
                    ...
                    if (face_db.empty()) {
                        // DB is empty, cannot match. Force Unknown.
                        ...
                        for (const auto& user : face_db) {
                            ...
                            if (similarity > highest_similarity) { 
                                highest_similarity = similarity; strcpy(best_match, user.name); 
                            }
                        }

                        if (highest_similarity > 0.6f) {
                            // MATCH! Unlock door and notify UI
                            char msg[64]; 
...
esp_err_t ws_handler(httpd_req_t *req) {
    ...
    // Handle Text Commands from the Web UI
    if (ws_pkt.type == HTTPD_WS_TYPE_TEXT) {
        ...
        if (strncmp(text, "CMD:MODE_ENROLL", 15) == 0) {
            ...
            ESP_LOGI(TAG, "Mode Switched: ENROLLMENT");
        } else if (strncmp(text, "CMD:MODE_SECURITY", 17) == 0) {
            current_mode = MODE_SECURITY;
            ESP_LOGI(TAG, "Mode Switched: SECURITY");
            ...
            broadcast_ws_msg(HTTPD_WS_TYPE_TEXT, (uint8_t*)msg, strlen(msg));
            ESP_LOGI(TAG, "Database Wiped!");
        } else if (strncmp(text, "CMD:UNLOCK", 10) == 0) {
            const char* msg = "UNLOCK";
            broadcast_ws_msg(HTTPD_WS_TYPE_TEXT, (uint8_t*)msg, strlen(msg));
            ESP_LOGI(TAG, "Manual Unlock Triggered by Web UI");
        }
    ...
    // Handle Binary Image from the ESP32-CAM
...
httpd_handle_t start_webserver() {
    ...
    config.max_open_sockets = 7; // Maximum allowed by default ESP-IDF config

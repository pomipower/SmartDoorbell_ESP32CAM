...
void loop() {
  ...
  http.setTimeout(20000); // 20s — face_recognition inference can be slow

  int httpCode = http.POST(fb->buf, fb->len);
  esp_camera_fb_return(fb); // Return buffer immediately after POST

  if (httpCode == 200) {
    // Response format:  "LINE1\nLINE2"
    ...
    if (response.startsWith("Welcome")) {
      // ── ACCESS GRANTED ────────────────────────────────────────
      // LED steady for the same duration as the relay unlock
      // (runs concurrently with relay via both happening in sequence)
      unlockDoor(RELAY_UNLOCK_MS); // Relay LOW for 3s, then HIGH
      ledSteady(300);              // Brief confirmation flash after re-lock

    } else {
      // ── ACCESS DENIED or SCAN FAILED ─────────────────────────
      ledBlink(4, 80, 80); // 4 rapid blinks = rejected
    }

  } else {
    // Server error or unreachable
    ledBlink(2, 200, 200); // 2 slow blinks = comms error
  }

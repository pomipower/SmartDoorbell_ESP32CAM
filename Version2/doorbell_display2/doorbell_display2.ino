...
uint32_t statusBgColor(const String& status) {
  if (status == "SUCCESS") return CLR_BG_SUCCESS;
  if (status == "DENIED")  return CLR_BG_DENIED;
...
void checkForNewEvent() {
  ...
  // Fetch and render the annotated visitor photo
  ...
  // Draw status bar over the bottom of the screen (or text fallback)
